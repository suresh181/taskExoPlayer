package com.example.task.tamil
...
import com.example.task.databinding.ActivityTamilBinding
...
class TamilActivity : AppCompatActivity() {
    private lateinit var apiClient: ApiClient
    private lateinit var binding: ActivityTamilBinding
...
//        startService(Intent(this, Service::class.java))
        ...
        binding = ActivityTamilBinding.inflate(layoutInflater)
        setContentView(binding.root)

        apiClient = ApiClient()
        val post = intent.getStringExtra("TAMIL")
        binding.tvPost.text = post

        recyclerView = binding.recyclerView
        recyclerView?.layoutManager = GridLayoutManager(this@TamilActivity, 2)
        getDetails()
        adapter = RecyclerView(this@TamilActivity, mList)
        recyclerView?.adapter = adapter
    }
    fun getDetails() {

        apiClient.getClient().tamilDetails()?.enqueue(object : Callback<ResponseTamil?> {
            override fun onResponse(
                ...
                val errorHandler = CoroutineExceptionHandler { _, exception ->
                    AlertDialog.Builder(this@TamilActivity).setTitle("Error")
